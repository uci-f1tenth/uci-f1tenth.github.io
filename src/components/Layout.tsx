...
function Header() {
  ...
  return (
    <header className="sticky top-0 w-full z-10 flex flex-col gap-2.5 items-end px-5 py-2.5">
      {/* always visible content */}
      <div className="w-full flex justify-between items-center">
        <img src="/assets/F1TenthUCIlogo.png" alt="logo" className="w-[7.5rem]" />
        <Bars3Icon onClick={ () => { setIsMenuOpen(!isMenuOpen); }} className="w-12 text-white" />
      </div>
      <AnimatePresence>
        { isMenuOpen && (
          <motion.ul 
...
function Footer() {
  return (
    <footer className="w-full flex flex-col gap-10 items-center py-12 bg-neutral-800">
      <h3 className="font-black text-2xl text-white">JOIN OUR SERVER</h3>
